...
var (
	...
	ErrFuncCallUnsupporteBackend = errors.New("backend does not support function calls")
	...
	ErrNotAGoFunction            = errors.New("not a go functoin")
)
...
type functionCallState struct {
	...
	argmem []byte
	// retvars contains the return variables, or the panic, after the function call terminates
	retvars []*Variable
	// loadCfg is the load configuration used to load return values
	loadCfg *LoadConfig
...
func CallFunction(p Process, expr string, loadCfg *LoadConfig) error {
	bi := p.BinInfo()
	if !p.Common().fncallEnabled {
		return ErrFuncCallUnsupporteBackend
	}
	...
	*fncall = functionCallState{}
	...
	// write the desired argument frame size at SP-16 (the extra -8 is the saved PC)
	if err := writePointer(bi, g.Thread, regs.SP()-16-8, uint64(len(argmem))); err != nil {
		return err
	}
...
func fncallLog(fmtstr string, args ...interface{}) {
	...
	logger := logrus.New().WithFields(logrus.Fields{"layer": "proc", "kind": "fncall"})
...
func (fncall *functionCallState) step(p Process) {
	...
	case debugCallAXReadReturn:
		...
		if fncall.loadCfg == nil || fncall.panicvar != nil {
			break
		}
		...
		fakeFunctionEntryScope(scope, fncall.fn, int64(regs.SP()), regs.SP()-0x8)
		...
		loadValues(fncall.retvars, *fncall.loadCfg)

	case debugCallAXReadPanic:
		// read panic value from stack
		if fncall.loadCfg == nil {
			return
		}
		fncall.panicvar, err = readTopstackVariable(thread, regs, "interface {}", *fncall.loadCfg)
		if err != nil {
			fncall.err = fmt.Errorf("could not get panic: %v", err)
			break
		}
		fncall.panicvar.Name = "~panic"
		fncall.panicvar.loadValue(*fncall.loadCfg)
