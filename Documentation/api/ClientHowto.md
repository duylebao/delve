...
The `dlv` binary build by our `Makefile` contains both the backend and a
...
1. delve will work much worse unless the binary is compiled specifying `-gcflags='all=-N -l'`
2. your users *do want* to debug their tests.

It would also be nice to your users if you provided a way to attach to a running process, like `dlv attach` does.
...
Once you have a running headless instance you can connect to it and start sending command. Delve's protocol is built on top of the [JSON-RPC](http://json-rpc.org) specification.
...
If you want to let your users specify a breakpoint on a function selecte
...
argument, this specifies which stack frame you are interested into. If you
are interested in the topmost stack frame of the current goroutine (or
thread) use: `EvalScope{ GoroutineID: -1, Frame: 0 }`.

More informations on the expression language interpreted by RPCServer.Eval
